...
class UserAPIController extends AppBaseController
{
    ...
    public function login(Request $request)
    {
        ...
        // if($this->addEmailToList($request->email)!=0){
        //     return response()->json(['status' => '201', 'message' => trans('auth.verify_email')]);
        // }else{
            if (!$validate->passes()) {
                ...
                if (Setting::get('mail_verification') == 'off') {
                    ...
                    if ($user != null) {
                        if ($user->email_verified == 1) {
                            ...
                            return response()->json(['status' => '202', 'message' => trans('messages.verify_mail')]);
                        }
                    }
                }
            }
            ...
            if ($canLogin && Auth::attempt(['email' => $nameoremail, 'password' => $request->password], $remember)) {
                $token = str_random(30);
                // dd($deviceToken);
                $result = User::where('id', $user->id)->update(['remember_token'=> $token, 'deviceToken' => $deviceToken]);
